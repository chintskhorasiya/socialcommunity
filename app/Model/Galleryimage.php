...
class Galleryimage extends AppModel {
    ...
    public $validate = array(
        'title' => array(
            'between' => array(
                'rule' => array('lengthBetween', 2, 255),
                'message' => 'Title can not be empty, And length should be between 2 to 255',
            )
        ),
        'source' => array(
     ...
     * @param string
     */
    public $uploadDir = 'img/uploads/galleryimages_images';
    ...
    public function processUpload($check=array()) {
        ...
        if (!empty($check['source']['tmp_name'])) {
            ...
            $res3_images = $images_move_dir . 'front_big_'.$new_image_name;
            ...
            // file successfully uploaded
                ...
                $resize2Obj->resize($images, $res2_images, '356', '237','100');
                chmod($res2_images,0777);

                $resize3Obj = new resize_image();
                $resize3Obj->resize($images, $res3_images, '640', '426','100');
                chmod($res3_images,0777);
