...
<div class="breadcrumb">
	<section class="article-breadcrumb">
		<?php
		$category_breadcrumb = '<span class="br-arrow">» </span> <a href="'.DEFAULT_URL.'gallery">Photo Gallery</a>';
		?>
		<a href="<?=DEFAULT_URL?>" title="Home"> Home</a><?=$category_breadcrumb;?><span class="br-arrow">» </span><?php echo $gallery_page_title; ?>
	</section>
</div>
<div class="left-part"> <!-- left-part start -->
			<div class="item col-md-12">
    			<?php
    			if(!empty($gallery_page_images))
    			{
    				$imgs_arr = explode(',', $gallery_page_images);
    				if(count($imgs_arr) > 0)
    				{
    				?>
        	...
        	<div class="inner-list-dec">
			    <p class="dba_pdate">Last Modified - <?php echo $gallery_page_modified; //date('M d, Y, g:i A', $news_page_modified); ?></p>
				<h1><?=$gallery_page_title?></h1>
				<div style="word-wrap: break-word;"><?php echo $gallery_page_content; ?></div>
			</div>	
           	<?php
           	...
           	<div class="col-md-12 cat-list-grid">
           		<h2 class="main-title yellow">More Galleries</h2>
           		<span class="yellow-border"></span>
           		<div class="clear"></div>
           		<?php
           		if(count($gallery_page_moregallery) > 0){
           		
           			foreach ($gallery_page_moregallery as $moregallery_key => $moregallery_data) {
           			?>
           				<div class="col-md-6">
			        		<a href="<?=DEFAULT_URL.'gallery-photos/'.$moregallery_data['Gallery']['slug']?>">
			        			<?php
			        			if(!empty($moregallery_data['Gallery']['images'])){
									$latest_gallery_images = explode(',', $moregallery_data['Gallery']['images']);
			        			...
			        			<h3><?php echo mb_substr($moregallery_data['Gallery']['title'], 0, 80); ?></h3>
								<p>Click here to Watch Gallery</p>
							</a>
			         	</div>
           			<?php
           			}
